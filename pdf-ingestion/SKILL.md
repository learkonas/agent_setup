...
- Cache the PDF at a prompt-caching breakpoint if the API supports it. Native PDFs are large — every uncached turn costs full input price on the whole doc.
...
If the PDF is a spec for ongoing work, extract it into a markdown file once — later turns should re-read prose, not re-scan the PDF.
