...
when_to_use: standing up an evaluator/critic agent, noticing evaluator scores drift upward across many iterations, grading skill/PR/diff output with an LLM and wanting reproducible verdicts
---

# Evaluator Calibration

An evaluator agent that reads the generator's reasoning drifts lenient. The generator explains why the code is good; the evaluator, priming on that prose, starts nodding along. After enough iterations the "skeptical critic" is a rubber stamp. Evaluator leniency is the characteristic failure mode of generator/evaluator setups.
...
- You're building a critic/evaluator/judge agent that runs repeatedly.
...
6. **Log verdict distributions.** Track pass rate per criterion over time. A criterion that goes from 40% pass to 90% pass without a spec change is drift, not improvement.

7. **Spot-check with a held-out fail.** Every ~10 evaluation cycles, feed the evaluator an artifact from your example set that you know fails. If it passes, the calibration has decayed — regenerate the example set from recent real runs.
